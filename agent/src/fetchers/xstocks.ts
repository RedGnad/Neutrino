import type { AssetSymbol, MarketSnapshot } from '../types.ts';

/**
 * On-chain xStocks data via Fluxion DEX on Mantle.
 *
 * Day-1 stub: returns hand-tuned mock snapshots so the rest of the pipeline
 * (scoring → decision → on-chain log) is exercisable without waiting for
 * Fluxion ABI/subgraph integration. Replace `fetchFluxionPool` with the real
 * RPC reads once a dev validates the Fluxion contract layout (R1 in SPEC.md).
 */
...
const MOCK_BASE: Record<AssetSymbol, MockBase> = {
  ...
  GOOGLx: { fallbackPrice: 175,  basisDriftPct: 0.002, spreadOpen: 13,  spreadClosed: 75,  volume24hUsd: 160_000,   volatility: 0.40 },
  MSTRx:  { fallbackPrice: 1450, basisDriftPct: 0.006, spreadOpen: 25,  spreadClosed: 140, volume24hUsd: 90_000,    volatility: 1.10 },
