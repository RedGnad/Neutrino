import type { Action, AssetMetadata, Decision, MarketSnapshot, UserPolicy } from '../types.ts';
import { computeRiskScore, type BasisStats } from '../scoring/risk-score.ts';
...
export interface DecideInput {
  ...
  /** Optional LLM-produced explanation. Falls back to a deterministic string if absent. */
  reason?: string;
}

/**
 * Rules engine — fully deterministic. The LLM is *only* asked for the `reason`
 * string. The action is computed from inputs alone so that decisions are
 * auditable and reproducible from the on-chain reasonHash.
 */
...
function pickAction(meta: AssetMetadata, snap: MarketSnapshot, policy: UserPolicy, risk: number): Action {
  // Hard rule: equity outside market hours under "no after-hours" → PAUSE.
  if (
    meta.kind === 'tokenized_equity' &&
    !snap.marketOpen &&
    policy.blockAfterHoursEquity
  ) {
    return 'PAUSE';
  }
  ...
  // Yield-first policy biases toward MOVE_TO_STABLE_YIELD on borderline equities.
